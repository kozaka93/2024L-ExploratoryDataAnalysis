...
# Odp.: Bugatti z 2008 roku i 2009 mają najwyższe średnie spalanie w mieście i wynosi ono 29.4 L/100km
...
data %>% 
  ...
  group_by(Make) %>% 
  summarise(price_median = median(MSRP, na.rm = TRUE)) %>% 
  arrange(-price_median)

# jak rozumiem mam tu porównać średnie parametry silnika marki Volskwagen
# i marki Hyundai dla samochodów z tej kategorii.

data %>% 
  filter(Market.Category == "Luxury,Performance", Year >= 2000,
         Make == "Hyundai" | Make == "Volkswagen") %>%
  group_by(Make) %>% 
...
# a Hyundai 6 cylindrów i 311 koni mechanicznych.
...
data %>% 
  ...
  mutate(fuel_usage_city = 1 / city.mpg * 3.785 / 1.609 * 100 ) %>%
  transmute(kwantyl_01 = quantile(fuel_usage_city, 0.1, na.rm=TRUE), 
         kwantyl_09 = quantile(fuel_usage_city, 0.9, na.rm=TRUE)) %>% 
  head(1)
