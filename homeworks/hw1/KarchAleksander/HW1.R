...
data %>% 
  group_by(Make, Year) %>% 
  summarise(CityL100km = 100/(mean(city.mpg, na.rm = TRUE)*1.609/4.54609)) %>% 
  arrange(-CityL100km)


# Odp.: Ex aequo Bugatti rocznik 2009 i 2008 - 35.3 L/100km
...
data %>%
  ...
  summarise(MotorwayL100km = 100/(mean(highway.MPG, na.rm = TRUE)*1.609/4.54609))
  

# Odp.:"flex-fuel (premium unleaded required/E85)"    all wheel drive 
# srednie spalanie wynosi 14.67747 L/100km
...
data %>% 
  group_by(Make, Model) %>% 
  summarise(diff = abs(100/(mean(highway.MPG, na.rm = TRUE)*1.609/4.54609)-100/(mean(city.mpg, na.rm = TRUE)*1.609/4.54609))) %>% 
  arrange(-diff)

# Odp.: Ferrari Enzo, 16.8 L/100km
