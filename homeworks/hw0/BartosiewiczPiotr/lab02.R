...
select(starwars_new, name, sex, mass)
select(starwars_new, name, mass, sex) #różna kolejność

wybierz <- c('name', 'skin_color')
select(starwars_new, wybierz)
...
wybierz <- c(1,2,3,400)
select(starwars_new, all_of(wybierz))
select(starwars_new, any_of(wybierz))
# b) wybór wierszy ---> filter()

filter(starwars, eye_color == "blue")

filter(starwars, eye_color == "blue" & hair_color == "blond") #logiczne i

filter(starwars, eye_color == "blue"| hair_color == "blond") # logiczne lub
...
# zadanie 
starwars %>%
  mutate(hair_new = case_when(hair_color == "blond"~"blond",
                   hair_color=="brown"~"brown",
                   TRUE~"Other"))%>%
  select(name,hair_new)%>%
  head()
...
# czyli ponizej 5 zliczen wrzuca ci wszystko do jednego worka
