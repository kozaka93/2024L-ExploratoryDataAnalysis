...
#install.packages("dplyr", "ggplot2", "ggrepel", "lubridate", "forcats")
library(dplyr)
library(ggplot2)
library(ggrepel)
library(lubridate)
...
spotify %>%
  ...
  ggplot(aes(x = as.factor(artist_count), y = sum_streams)) +
...
spotify %>%
  ...
  geom_violin(fill = "orange", alpha = 0.8, color = "lightyellow") +
...
spotify %>%
  ...
  ggplot(aes(x = fct_reorder(as.factor(released_year), -released_year), y = streams, label = track_name)) +
  geom_col(fill = "brown", color = "lightyellow") +
  # geom_label_repel() 
  geom_text(position = position_stack(vjust = 0.5), size = 4, color = "white") +
  ylim(0, 4.2e9) +
  labs(title = "Najpopularniejsza piosenka wydana w danym roku",
       x = "Rok",
       y = "Liczba odtworzeń") +
  scale_y_continuous(labels = comma) +
  coord_flip()
